...
| Module              | Change                                                                                         |
| ------------------- | ---------------------------------------------------------------------------------------------- |
| `build.sbt`         | `UNI_VERSION` 2026.1.9 → 2026.1.11 (Node sync HTTP + auto-registered HTTP channel factory)     |
| `wvlet-lang`        | New shared `Trino` + `TrinoConfig`                                                             |
...
### No per-platform compat needed (after uni 2026.1.11)

The first cut of this PR shipped a `TrinoCompat` trait per platform that called `Http.setDefaultChannelFactory(...)` at class-init. uni 2026.1.9 auto-registered the factory inside `HttpCompat`, but that object only loaded when the exception classifier needed it — too late if `Trino.execute` ran before any error path. The reproduction was `NoOpChannelFactory.newChannel` throwing "No HttpChannel implementation available" on the first `Http.client.newSyncClient`.

[wvlet/uni#549](https://github.com/wvlet/uni/pull/549) (in 2026.1.11) made `Http.defaultChannelFactory` read from `HttpCompat.defaultHttpChannelFactory` during its own object init, so the platform-specific factory is in place before any caller touches `Http.client`. The bump from 2026.1.10 to 2026.1.11 removed `TrinoCompat.scala` from all three platforms and the `extends TrinoCompat` / `installHttpFactory()` plumbing from `Trino.scala` — about -25 LOC.
