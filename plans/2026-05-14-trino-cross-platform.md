...
- **Drop `TrinoCompat` once uni 2026.1.11 lands** — [wvlet/uni#549](https://github.com/wvlet/uni/pull/549) makes `Http.defaultChannelFactory` auto-initialize from `HttpCompat.defaultHttpChannelFactory`, so the per-platform `TrinoCompat` plumbing becomes redundant. Follow-up: bump uni, delete `TrinoCompat.scala` in all three platforms, drop `extends TrinoCompat` and the `installHttpFactory()` call from `Trino.scala`. About -25 LOC.
