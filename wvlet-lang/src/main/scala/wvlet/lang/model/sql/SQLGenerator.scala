...
case class SQLGeneratorConfig(indent: Int = 2)
...
//      case d: DDL      => printDDL(d)
  ...
  private def collectFilterExpression(stack: List[Relation]): Seq[Expression] =
    ...
    stack
      .reverse
      .collect { case f @ Filter(in, filterExpr, _) =>
        filterExpr
      }
  ...
  private def printSelection(s: UnaryRelation, context: List[Relation]): String =
    ...
    s match
      case a @ SQLSelect(_, _, groupingKeys, having, _, _) =>
        ...
        having.map { h =>
          b += "HAVING"
          b += printExpression(h.filterExpr)
        }
      case _ =>
    b.result().mkString(" ")
  ...
  def printRelation(r: Relation, context: List[Relation] = List.empty): String =
    r match
      ...
      case q: Query =>
        q.body match
          case s: Selection =>
            printSelection(s, q :: context)
          case _ =>
            printQuery(q, q :: context)
      case Distinct(in, _) =>
        ...
        val c =
          cond match
            ...
            case JoinUsing(columns, _) =>
              s" USING (${columns.map(_.sqlExpr).mkString(", ")})"
