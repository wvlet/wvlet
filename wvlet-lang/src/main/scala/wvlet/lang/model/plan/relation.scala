...
case class Unpivot(child: Relation, unpivotKey: UnpivotKey, span: Span) extends UnaryRelation:
  ...
  override lazy val relationType: RelationType =
    ...
    val unpivotColumnType: DataType = inputFields
      .find(f => f.name.name == unpivotKey.getValueColumnName.fullName)
      .map(_.dataType)
    ...
    ProjectedType(
      ...
      nonPivotColumns ++
        List(
          NamedType(unpivotKey.unpivotColumnName.toTermName, StringType),
          NamedType(unpivotKey.getValueColumnName.toTermName, unpivotColumnType)
        ),
      child.relationType
    )
...
case class UnpivotKey(
    valueColumnName: Option[Identifier],
  ...
  def getValueColumnName: Identifier = valueColumnName.getOrElse(
    Identifier(List(QName("value", span)), span)
  )
