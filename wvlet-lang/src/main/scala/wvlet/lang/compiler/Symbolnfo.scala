...
/**
  * Symbol info for flow definitions. Flows are named data/control workflows containing stages.
  *
  * @param owner
...
case class FlowSymbolInfo(
    ...
    override val name: Name,
    compilationUnit: CompilationUnit
) extends SymbolInfo(SymbolType.FlowDef, symbol, owner, name, DataType.EmptyRelationType):
  override def toString: String = s"flow ${owner}.${name}"

/**
  * Symbol info for stage definitions within a flow. Stages are named steps in a workflow.
  *
  * @param owner
  *   The owning symbol (typically the flow)
  * @param symbol
  *   The symbol for this stage
  * @param name
  *   The name of the stage
  * @param tpe
  *   The relation type of this stage's output
  */
case class StageSymbolInfo(
    override val owner: Symbol,
    override val symbol: Symbol,
    override val name: Name,
    override val tpe: DataType
) extends SymbolInfo(SymbolType.StageDef, symbol, owner, name, tpe):
  override def toString: String = s"stage ${owner}.${name}: ${dataType}"
