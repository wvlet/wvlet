...
class CompletionProviderTest extends UniTest:
  ...
  test("should offer function defs with their signature as the completion detail"):
    // Engine-native defs (e.g. generated by `wvlet catalog import` in functions.wv) surface
    // in completion with a `(args): ret` signature
    val src =
      """def date_trunc2(a1: string, a2: timestamp) in duckdb: timestamp = native
        |
        |from [[1]] as t(x)
        |""".stripMargin
    val items  = complete(src, src.length)
    val fnItem = items.find(_.label == "date_trunc2")
    fnItem.map(_.kind) shouldBe Some(CompletionItemKind.Function)
    fnItem.map(_.detail) shouldBe Some("(a1: string, a2: timestamp): timestamp")
