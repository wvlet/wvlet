...
class DateTimeFunctionParserTest extends AirSpec:
  ...
  test("should parse date function calls correctly") {
    ...
    expr shouldMatch {
      case FunctionApply(UnquotedIdentifier("date", _), _, _, _) =>
    }
  }
  ...
  test("should parse time function calls correctly") {
    ...
    expr shouldMatch {
      case FunctionApply(UnquotedIdentifier("time", _), _, _, _) =>
    }
  }
  ...
  test("should parse timestamp function calls correctly") {
    ...
    expr shouldMatch {
      case FunctionApply(UnquotedIdentifier("timestamp", _), _, _, _) =>
    }
  }
