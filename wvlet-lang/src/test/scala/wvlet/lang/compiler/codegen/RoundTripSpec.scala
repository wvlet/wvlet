...
abstract class RoundTripSpec(path: String, ignoredSpec: Map[String, String] = Map.empty)
  ...
  for unit <- CompilationUnit.fromPath(path) do
    ...
    test(s"Roundtrip ${specName}") {
      ...
      trace(rewrittenPlan.pp)

      val config = CodeFormatterConfig()
      val g = if unit.isSQL then SqlGenerator(config) else WvletGenerator(config)
      ...
      {
        val reformattedUnit            =
          if unit.isSQL then CompilationUnit.fromSqlString(str) else CompilationUnit.fromWvletString(str)
...
class WvletRoundTripSpecSqlTPCH extends RoundTripSpec("spec/sql/tpc-h")
