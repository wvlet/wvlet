...
trait SourceIOCompat:
  // Bind uni's Native FileSystem impl once at class-load time so callers don't need to know
  // about FileSystemInit. Same trait body lives in the JVM folder — uni's `IO` API works
