...
### Fixed
- The language server in 2026.1.0 was non-functional: the release was packaged
  in CI without the Scala.js compiler bundle (which is generated by sbt, not
  checked into git), so the published extension silently degraded to syntax
  highlighting only. The release workflow now builds the compiler bundle before
  packaging, and packaging fails hard if the bundle is missing so a broken
  package can never be produced silently again.
