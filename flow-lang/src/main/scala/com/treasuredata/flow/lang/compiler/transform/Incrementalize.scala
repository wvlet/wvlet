  ...
  object IncrementalizeSimpleScan extends RewriteRule:
    ...
    private def isSimpleScan(plan: LogicalPlan, context: Context): Boolean =
      plan match
        ...
        case _: TableRef              => true
        case _: Values                => true
        case p: ParenthesizedRelation => isSimpleScan(p.child, context)
        case a: AliasedRelation       => isSimpleScan(a.child, context)
        case r: RelScan =>
          context.compileUnit.findRelationRef(r.name) match
            case Some(rel) => isSimpleScan(rel, context)
            case _         => false
        case _ => false
