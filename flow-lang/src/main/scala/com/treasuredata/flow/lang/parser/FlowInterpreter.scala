...
class FlowInterpreter extends FlowLangBaseVisitor[Any] with LogSupport:
  ...
  override def visitQuery(ctx: QueryContext): Relation =
    ...
    if Option(ctx.selectExpr()).isDefined then r = interpretSelect(r, ctx.selectExpr())

    Query(r, getLocation(ctx))

  private def interpretSelect(r: Relation, ctx: SelectExprContext): Relation =
    val selectItems: List[Attribute] =
      Option(ctx.selectItemList()) match
        case Some(lst) =>
  ...
  override def visitAliasedRelation(ctx: AliasedRelationContext): Relation =
    val r: Relation = ctx.relationPrimary() match
      ...
      case u: UnnestContext =>
        ...
        Unnest(
          u.primaryExpression().asScala.toSeq.map(x => interpretExpression(x)),
          withOrdinality = ord,
          getLocation(ctx)
        )
      ...
      if ctx.groupBy() == null then
        ...
        having match
          case Some(h) =>
            ...
            else Aggregate(inputRelation, selectItem, List.empty, Some(h), getLocation(ctx))
          case None =>
            // Regular aggregate
            val p = Project(inputRelation, selectItem, getLocation(ctx))
            if distinct then Distinct(p, p.nodeLocation)
            else p
      else
        // aggregation
        val gb = ctx.groupBy()
  ...
  override def visitExpression(ctx: ExpressionContext): Expression =
    ...
    b match
      ...
      case ln: LogicalNotContext =>
        if ln.NOT() != null then Not(expression(ln.booleanExpression()), getLocation(ctx))
        else throw unknown(ln)
