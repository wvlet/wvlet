...
class TestingDeltaLakePlugin(localFileSystemRootPath: Path) extends DeltaLakePlugin:
  override def getConnectorFactories: java.lang.Iterable[ConnectorFactory] =
    ImmutableList.of(new ConnectorFactory():
      def getName: String = DeltaLakeConnectorFactory.CONNECTOR_NAME

      def create(catalogName: String, config: java.util.Map[String, String], context: ConnectorContext): Connector =
...
class TestDeltaLakeModule extends com.google.inject.Module:
  override def configure(binder: Binder): Unit =
    newMapBinder(binder, classOf[String], classOf[TransactionLogSynchronizer])
...
class FileTestingTransactionLogSynchronizer @Inject (fileSystemFactory: TrinoFileSystemFactory)
  ...
  def write(session: ConnectorSession, clusterId: String, newLogEntryPath: Location, entryContents: Array[Byte]): Unit =
    try
