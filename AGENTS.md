...
- Root SBT multi-project for Scala 3; JS/TS pieces use pnpm workspaces (see `pnpm-workspace.yaml`).
...
- UI dev: `pnpm run ui` (runs `wvlet-ui-main` via Vite). Playground: `pnpm run playground`. Build UI: `pnpm run build-ui`. Install deps once with `pnpm install`.
...
- Some JS packages use Jest; run via the workspace filter (e.g., `pnpm --filter @wvlet/prismjs-wvlet test`).
...
- Prefer scoped tests to keep feedback fast: `./sbt langJVM/test`, `./sbt runner/test`, `./sbt server/test`, or `./sbt "langJVM/testOnly *SqlParserTest"`. For UI workspaces, also `pnpm run build-ui`.
