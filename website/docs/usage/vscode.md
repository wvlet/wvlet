...
The language server loads the `.wv` sources of your workspace folder (the first folder in a
multi-root workspace), including the `catalog/` folder generated by
[`wvlet catalog import`](./catalog-import.md). After importing your database catalog once,
table references such as `from sales.orders` resolve offline in the editor: diagnostics
validate them, hover shows their columns, and column names complete — no live database
connection needed.

Note: source discovery follows the compiler's folder scan, which descends into folders that
contain `.wv` files (plus the `catalog/` folder, which is always loaded). Keep at least one
`.wv` file at the workspace root — or open the folder that holds your queries — so nested
sources are found.

## Known Limitations

- **Schema for database tables**: Tables get completion and hover only when their schema is
