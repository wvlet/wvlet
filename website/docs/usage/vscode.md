...
The language server loads all `.wv` sources of your workspace folder, including the
`catalog/` folder generated by [`wvlet catalog import`](./catalog-import.md). After importing
your database catalog once, table references such as `from sales.orders` resolve offline in
the editor: diagnostics validate them, hover shows their columns, and column names complete —
