...
class StaticCatalogE2ETest extends AirSpec:
  ...
  test("end-to-end: import catalog and compile with it") {
    withTempCatalog { tempPath =>
      ...
      try
        ...
        connector.executeUpdate("CREATE TABLE employees (id INT, name VARCHAR, salary DECIMAL(10,2))")
        connector.executeUpdate("INSERT INTO employees VALUES (1, 'Alice', 50000.00), (2, 'Bob', 60000.00)")
        ...
        // Write catalog files
        val schemasToWrite = if schemas.isEmpty then
          List(Catalog.TableSchema(Some(catalogName), "main", "Main schema"))
        else
          schemas.toList
        Files.writeString(catalogDir.resolve("schemas.json"), CatalogSerializer.serializeSchemas(schemasToWrite))
        Files.writeString(catalogDir.resolve("main.json"), CatalogSerializer.serializeTables(tables.toList))
        Files.writeString(catalogDir.resolve("functions.json"), CatalogSerializer.serializeFunctions(functions.toList))
        
      finally
        connector.close()
      
      // Phase 2: Load static catalog and verify
      val loadedCatalog = StaticCatalogProvider.loadCatalog(catalogName, dbType, tempPath.resolve("catalog"))
  ...
  test("static catalog with multiple schemas") {
    withTempCatalog { tempPath =>
      ...
      Files.writeString(catalogDir.resolve("public.json"), CatalogSerializer.serializeTables(publicTables))
      Files.writeString(catalogDir.resolve("analytics.json"), CatalogSerializer.serializeTables(analyticsTables))
