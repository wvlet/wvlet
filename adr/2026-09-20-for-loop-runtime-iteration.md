...
- **Query iterables bind the first column only**, run once before the first iteration, and fetch every row through `executeQueryAllRows`: the runner's row limit (default 40) is a display limit and must not decide how many iterations run. Row/struct binding is deferred.
...
- Iterations are sequential and fail-fast. There is deliberately no iteration cap: a loop is already bounded by its iterable, and a fixed limit (an early draft had 10,000) is one more setting to manage that fails legitimate large backfills. Parallel iteration and loops inside `flow` are separate designs.
...
- `RunnerSpecNeg` does not assert that an error occurred, so `spec/neg/for-loop-non-array.wv` only documents the behavior; `ForLoopTypingTest` asserts the `INVALID_LOOP_ITERABLE` error.
